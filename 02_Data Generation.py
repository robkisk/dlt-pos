...
# MAGIC %md
# MAGIC
# MAGIC ## Step 1: Assemble Inventory Change Records
# MAGIC
# MAGIC - The inventory change records represent events taking place in a store location which impact inventory.
# MAGIC - These may be sales transactions, recorded loss, damage or theft, or replenishment events.
# MAGIC - In addition, Buy-Online, Pickup In-Store (BOPIS) events originating in the online store and fulfilled by the physical store are captured in the data.
# MAGIC - While each of these events might have some different attributes associated with them, they have been consolidated into a single stream of inventory change event records.
# MAGIC - Each event type is distinguished through a change type identifier.
# MAGIC
# MAGIC - A given event may involve multiple products (items).
# MAGIC - By grouping the data for items associated with an event around the transaction ID that uniquely identifies that event,
# MAGIC     - the multiple items associated with a sales transaction or other event type can be efficiently transmitted:
...
inventory_change = (
    ...
    .withColumn(
        "trans_id", f.expr("substring(trans_id, 2, length(trans_id)-2)")
    )  # remove surrounding curly braces from trans id
    .withColumn(
        "item", f.struct("item_id", "quantity")
    )  # combine items and quantities into structures from which we can build a list
...
# MAGIC %md
# MAGIC ## Step 2: Assemble Inventory Snapshots
...
# MAGIC
# MAGIC - With each snapshot, we capture basic information about the store, item, and quantity on-hand along with the date time and employee associated with the count.
# MAGIC - So that the impact of inventory snapshots may be more rapidly reflected in our streams, we simulate a complete recount of products in a store every 5-days.
# MAGIC - This is far more aggressive than would occur in the real world but again helps to demonstrate the streaming logic:
...
inventory_snapshots = spark.read.csv(
    inventory_snapshot_files,
    header=True,
    timestampFormat="yyyy-MM-dd HH:mm:ss",
...
# MAGIC %md
# MAGIC - To coordinate the transmission of change event data with periodic snapshots,
# MAGIC     - the unique dates and times for which snapshots were taken within a given store location are extracted to a list.
# MAGIC - This list will be used in the section of code that follows:
# MAGIC
...
inventory_snapshot_times = (
    inventory_snapshots.select("date_time", "store_id")
    .distinct()
...
for blob in container_client.list_blobs(
    name_starts_with=config["inventory_snapshot_path"].replace(
        config["dbfs_mount_name"], ""
    )[1:]
):
...
if mode != "prod":
    inventory_change = inventory_change[
        :100
    ]  # limit the number of transactions here to shorten execution duration; this notebook is engineered to run for approx. 3 days without this limit
event_speed_factor = (
    10  # Send records to iot hub at <event_speed_factor> X real-time speed
)
...
for event in inventory_change:
    ...
    for snapshot_dt, store_id in inventory_snapshot_times_for_loop:  # for each snapshot
        ...
        if dt < snapshot_dt:  # (snapshot times are ordered by date)
            ...
            snapshot_pd = (
                inventory_snapshots.filter(
                    f.expr(
                        "store_id={0} AND date_time='{1}'".format(store_id, snapshot_dt)
                    )
                )
                .withColumn(
                    "date_time", f.expr("date_format(date_time, 'yyyy-MM-dd HH:mm:ss')")
                )  # force timestamp conversion to include
                .toPandas()
            )
            ...
            blob_client = blob_service_client.get_blob_client(
                container=config["storage_container_name"],
                blob=(
                    config["inventory_snapshot_path"].replace(
                        config["dbfs_mount_name"], ""
                    )[1:]
                    + "inventory_snapshot_{0}_{1}".format(
                        store_id, snapshot_dt.strftime("%Y-%m-%d %H:%M:%S")
                    )
                ),
            )
            ...
            print(
                "Loaded inventory snapshot for {0}".format(
    ...
    if len(event) > max_msg_size:
        ...
        for i, item in enumerate(items):  # for each item
            ...
            if (
                i + 1
            ) % 25 == 0:  # pause transmission every Xth item to avoid overwhelming IOT hub
                time.sleep(1)
