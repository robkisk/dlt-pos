...
# MAGIC %md
# MAGIC - This notebook provides instructions for setting up the cloud services on which subsequent notebooks depend.  It also captures various configuration settings required by those notebooks.
# MAGIC - You do not need to run this notebook separately as each notebook in the set calls this notebook to retrieve configuration data. **However**, you should read the environment setup instructions in this notebook carefully and update all configuration values as appropriate.
# MAGIC
...
secret = dbutils.secrets.get(
    scope="solution-accelerator-cicd", key="rcg_pos_iot_hub_conn_string"
)
...
config["storage_account_name"] = dbutils.secrets.get(
    "solution-accelerator-cicd", "rcg_pos_storage_account_name"
)
...
config["storage_account_access_key"] = dbutils.secrets.get(
    "solution-accelerator-cicd", "rcg_pos_storage_account_key"
)
...
conf_key_name = "fs.azure.account.key.{0}.blob.core.windows.net".format(
...
if not mount_exists:
    ...
    dbutils.fs.mount(
        source="wasbs://{0}@{1}.blob.core.windows.net".format(
            config["storage_container_name"], config["storage_account_name"]
        ),
...
# COMMAND ----------
display(dbutils.fs.ls(config["dbfs_mount_name"]))
...
config["inventory_change_store001_filename"] = (
    config["dbfs_mount_name"] + "/generator/inventory_change_store001.txt"
)
config["inventory_change_online_filename"] = (
    config["dbfs_mount_name"] + "/generator/inventory_change_online.txt"
)

# snapshot data files
config["inventory_snapshot_store001_filename"] = (
    config["dbfs_mount_name"] + "/generator/inventory_snapshot_store001.txt"
)
config["inventory_snapshot_online_filename"] = (
    config["dbfs_mount_name"] + "/generator/inventory_snapshot_online.txt"
)
